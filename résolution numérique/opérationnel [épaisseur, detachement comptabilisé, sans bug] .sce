...
// constantes de Monod : 
k_1 = 500*10^(-5)*exp(-((T-T_opt_L)/(T_i_L - T_opt_L))^2) ; // constante de vitesse légionnelle-nutriment
k_2 = 0.2 ;
k_3 = 1000*10^(-5)*exp(-((T - T_opt_L)/(T_i_L - T_opt_L))^2) ; // constante de vitesse légionnelle-amibe
k_4 = 0.2 ;
k_5 = 900*10^(-5)*exp(-((T - T_opt_A)/(T_i_A - T_opt_A))^2) ; // constante de vitesse amibe-nutriment
k_6 = 0.2 ;
...
frequenceStockage = 6000; // on en garde 1/fréquenceStockage en fait
...
while (tCourant < T)
  ...
  //  end
    ...
    NCourant(round(eCourant/dz)+1,1) = valeurAuBordN;
...
// mise à jour des matrices de dérivée si l'épaisseur a augmenté
    if (round(eAvant/dz) <> round(eCourant/dz)) then
        matriceDeriveePremiere = matriceDeriveePremiere_ini; 
        ...
        if round(eAvant/dz) < round(eCourant/dz) then
            k = round(eAvant/dz) + 1
            while k <= round(eCourant/dz) 
            NCourant(k,1) = NCourant(round(eAvant/dz),1)
            k = k + 1
            end
            NCourant(round(eCourant/dz)+1,1) = valeurAuBordN;
        else
            k = round(eCourant/dz) + 1
