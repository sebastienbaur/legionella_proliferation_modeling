...
while (tCourant < T)
    ...
    epsilonCourant = epsilonCourant - dt * (epsilonCourant.*dudz(NCourant,LAvant,AAvant) + vitesseCourante.*(matriceDeriveePremiere*epsilonCourant))
...
//    BCourant = BCourant + dt * (D_b * (1/(dz*dz)) * (matriceDeriveeSeconde * BCourant) - (k_m_l * LAvant + k_m_a * AAvant) .* BCourant);
    ...
    epsilonCourant = epsilonCourant .* (matriceVitesse(indiceEpaisseur,:)');
    ...
    k = round(eAvant/dz);
        while (k <= indiceEpaisseur)
           epsilonCourant(k,1) = epsilonCourant(round(eAvant/dz),1);
    ...
    if modulo(i,frequenceStockage) == 0 then
        ...
        V = [V vitesseCourante];
        epsilon = [epsilon epsilonCourant]
    end
...
testConservationMasse = A/rho_A + L/rho_L + AMort/rho_A + LMort/rho_L + epsilon;
