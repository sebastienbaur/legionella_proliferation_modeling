...
S0(1,1) = 10;
S0(2,1) = 10;
valeurAuBordNutriment = 1;
conditionAuBordNutriment = zeros(M,1);
conditionAuBordNutriment(M,1) = D*valeurAuBordNutriment/(dz*dz);
...
matriceDeriveePremiere = surDiag - sousDiag; 
matriceDeriveePremiere(M,M-1) = 0;
...
while (tCourant < T)
    ...
    conditionAuBordL(M,1) = - LCourant(M-1,1);
    conditionAuBordL = conditionAuBordL .* deriveeVitesse(SCourant,LCourant,ACourant); // (u_m+1L_m+1 - u_m-1L_m-1)/(2*dz) = L_m-1*(u_m+1-u_m-1)/(2*dz) et la différence de vitesse est approximée par 2*dz*u_m
    conditionAuBordA = zeros(M,1);
    conditionAuBordA(M,1) = - ACourant(M-1,1);
    conditionAuBordA = conditionAuBordA .* deriveeVitesse(SCourant,LCourant,ACourant);
    
    LCourant = LCourant + dt * (monodL(SCourant, LCourant, ACourant).*LCourant - (1/(2*dz)) * matriceDeriveePremiere * (vitesseCourante .* LCourant)) + conditionAuBordL;
...
//  LCourant(1,1) = LCourant(2,1);
    ...
    ACourant = ACourant + dt * (monodA(SCourant, LAvant, ACourant).*ACourant - (1/(2*dz)) * matriceDeriveePremiere * (vitesseCourante .* ACourant)) + conditionAuBordA ;
...
//  ACourant(1,1) = ACourant(2,1);
    ...
    SCourant = SCourant + dt * (D * (1/(dz*dz) ) * (matriceDeriveeSeconde * SCourant) + consoNutri(SCourant,LAvant,AAvant)) + conditionAuBordNutriment; //.* SCourant);
...
//  SCourant(indiceEpaisseur,1)=SCourant(indiceEpaisseur-1,1);
    ...
    if modulo(i,frequenceStockage) == 0 then
       ...
       // indicesEpaisseur = [indicesEpaisseur indiceEpaisseur]
        ...
        m_S_relatif = [m_S_relatif m_S(1,i/frequenceStockage+2)/m_S(1,1)];
    end
